...
namespace InventoryDesktopApp
{
    internal static class Database
    {
        ...
        public static async Task<string> AddAdmin(string username, string password)
        {
            if (!await HasRegisteredAdmin())
            {
                using (SQLiteConnection myconnection = new SQLiteConnection(connectionString))
                {
                   ...
                   // string query = "INSERT INTO admin ('username','password','registered') VALUES (@username, @password,'Admin')";
        ...
        public static async Task<bool> UpdateAdminPassword(string username,string currentPassword, string newPassword)
        {
            // Verify current credentials
            if (!await LoginAdmin(username, currentPassword))
            {
                return false;
            }
                ...
                new SQLiteConnection(connectionString))
            {
                ...
                SQLiteCommand cmd =
                    new SQLiteCommand(query, myconnection);
